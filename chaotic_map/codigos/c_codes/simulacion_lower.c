...
/*
	...
	Data type <int> tiene 8 bytes, es decir 64 bits.
...
void inicializa( int a, int b ){		
	...
	_b = b;
	_power = (int)1<< _b;					// Calculamos el factor para la conversion
}

int setNumber( double v ){				// Convierte a punto fijo con truncamiento 
	return ( (int)(v*_power) );
}

double getNumber( int r ){				// Convierte de vuelta a double
	return ( (double)r/_power);
}

int multTrunc( int x, int y ){		// Multiplicacion con truncamiento
	long r;
	long a=0;
	long b=0;
	a = x;
